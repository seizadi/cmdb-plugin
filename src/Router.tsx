...
// FIXME - Change this annotation
export const isCmdbAvailable = (entity: Entity) =>
  Boolean(entity.metadata.annotations?.[LIGHTHOUSE_WEBSITE_URL_ANNOTATION]);

export const Router = () => (
  <Routes>
    <Route path="/" element={<AuditList />} />
    <Route path="/cmdb-audit/:id" element={<AuditView />} />
    <Route path="/cmdb-create-audit" element={<CreateAudit />} />
  </Routes>
);
...
export const EmbeddedRouter = (_props: Props) => {
  ...
  return (
    <Routes>
      ...
      <Route path="/cmdb-audit/:id" element={<AuditViewContent />} />
      <Route path="/cmdb-create-audit" element={<CreateAuditContent />} />
    </Routes>
  );
};
